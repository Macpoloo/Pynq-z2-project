...
entity calcola_AXILiteS_s_axi is
generic (
...
end entity calcola_AXILiteS_s_axi;
...
architecture behave of calcola_AXILiteS_s_axi is
    ...
    constant ADDR_O_DATA_0 : INTEGER := 16#20#;
    constant ADDR_O_CTRL   : INTEGER := 16#24#;
    ...
    signal int_o               : UNSIGNED(7 downto 0) := (others => '0');
        ...
        if (ACLK'event and ACLK = '1') then
            if (ACLK_EN = '1') then
                if (ar_hs = '1') then
                    ...
                    when ADDR_O_DATA_0 =>
                        rdata_data <= RESIZE(int_o(7 downto 0), 32);
                    when ADDR_C_DATA_0 =>
        ...
        if (ACLK'event and ACLK = '1') then
            if (ACLK_EN = '1') then
                if (w_hs = '1' and waddr = ADDR_O_DATA_0) then
                    int_o(7 downto 0) <= (UNSIGNED(WDATA(7 downto 0)) and wmask(7 downto 0)) or ((not wmask(7 downto 0)) and int_o(7 downto 0));
                end if;
            end if;
        end if;
    end process;
